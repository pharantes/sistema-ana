/**
 * Action Model - Manages event/action records with staff and cost tracking
 */
import mongoose from "mongoose";
const { Schema } = mongoose;

/**
 * Staff entry schema - Represents a staff member assigned to an action
 */
...
/**
 * Cost entry schema - Represents additional costs associated with an action
 */
...
/**
 * Action schema - Main schema for event/action records
 */
...
actionSchema.index({ 'staff.colaboradorId': 1 });
actionSchema.index({ 'costs.vendorName': 1 });
actionSchema.index({ date: -1, createdAt: -1 });
actionSchema.index({ startDate: -1, endDate: -1 });
