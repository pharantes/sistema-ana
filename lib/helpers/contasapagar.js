...
// Attach colaboradorLabel and full colaboradorData for rows with colaboradorId
export async function attachColaboradorLabel(rows) {
  try {
    ...
    const servs = await Colaborador.find({ _id: { $in: colaboradorIds } })
      .select("_id nome empresa codigo pix banco conta")
      .lean();
    ...
    const dataMap = new Map(
      servs.map((s) => [String(s._id), s])
    );
    for (const r of rows) {
      const sid = String(r?.colaboradorId || "");
      if (labelMap.has(sid)) {
        r.colaboradorLabel = labelMap.get(sid);
        r.colaboradorData = dataMap.get(sid);
      }
    }
  } catch {
...
// Link staffName rows to Colaborador by nome when colaboradorId is missing; attach colaboradorId, colaboradorLabel, and full data
export async function linkStaffNameToColaborador(rows) {
  try {
    ...
    const servsByName = await Colaborador.find({ nome: { $in: staffNames } })
      .select("_id nome empresa pix banco conta")
      .lean();
